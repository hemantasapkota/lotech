...
local ftext = import "data/hello.go"
...
local scroll_limits = {20, 36, 56, 76, 96, 116}
...
editor_input:Action(function(node, dt)
  if c.scrolling then
    return
  end
  ...
  if c.retCount == scroll_limits[scroll_index] then
    ...
    left_pane = left_pane:Translate(0, 0)
      :Tween{x=0, y = 2, time=0.3, action = function()
      end}
    container:Insert(left_pane)
  end
...
function show_esc_menu(menu_type)
  ...
  action_bars["TWEET"].action = function()
    ...
    t_url = t_url .. "I scored " .. c.wpm .. " #WPM typing #" .. string.lower(config.cur_lang) ..
...
--leaderboards
ty = ty - 0.4
local leaderbrd_lyr = lt.Layer():Translate(r_start + (r-r_start)/2, ty)
leaderbrd_lyr:Insert(panel_header("LEADERBOARD", -0.89, 0.89, config.sol_base0))
right_pane:Insert(leaderbrd_lyr)
