...
for i,lbl in ipairs(input_lbls) do
  action_bars[lbl].action =  function()
    ...
    show_modal(lbl, "_username", 15, 1.2)
  end
end
...
ls_body:Insert(make_action_bars(config.lang_opts.lang_labels):Translate(0, 0.15))
--make actions for each of the label
for _,lbl in ipairs(config.lang_opts.lang_labels) do
  action_bars[lbl].action = function()
