return [=[
fromm celery import task
from feedly.activity import Activity, AggregatedActivity

@task.task()
def fanout_operation(feed_manager, feed_class, user_ids, operation,
operation_kwargs):
    feed_manager.fanout(user_ids, feed_class, operation, operation_kwargs)
    return ''%d user_ids, %r, %r (%r)'' % (len(user_ids), feed_class, operation,
    operation_kwargs)

@task.task()
def fanout_operation_hi_priority(feed_manager, feed_class, user_ids, operation,
operation_kwargs):
    return fanout_operation(feed_manager, feed_class, user_ids, operation,
    operation_kwargs)

@task.task()
def fanout_operation_low_priority(feed_manager, feed_class, user_ids,
    ...
    for feed in feed_manager.get_feeds(user_id).values():
        ...
        for item in feed[:feed.max_length]:
            if isinstance(item, Activity):
                ...
                activities.extend(
                    [activity for activity in item.activities if
                              activity.actor_id in source_ids])
