...
if __name__ == '__main__':
        import argparse
        parser = argparse.ArgumentParser(description='Tests the true positive
        ...
        for nudepic in os.listdir(nudepath):
                ...
                pool.apply_async(testfile, (fname, ), {'resize':args.resize},
                nudecallback)
        ...
        if nudenum:
                print(''True positives: '', results['tp'], '/', nudenum, ' - ',
                       (results['tp']/nudenum)*100, '%', sep='')
        else:
                print(''True positives: N/A'')
        if nonnudenum:
                print(''False positives: '', results['fp'], '/', nonnudenum, ' -
                      ', (results['fp']/nonnudenum)*100, '%', sep='')
        else:
